...
int main() {
    ...
    for (int i = 0; i < num_warmup; i++) {
        reduce_kernel_v4<THREAD_PER_BLOCK><<<grid, block, smem_size>>>(d_in, d_out, N);
    }
    // repeat
    ...
    {
        std::cerr << "reduce_kernel_v4 check failed: sum=" << sum 
             << ", sum_out=" << sum_out 
             << ", diff=" << fabs(sum - sum_out) << std::endl;
    }
    std::cout << "reduce_kernel_v4: latency=" << seconds*1000.0 
